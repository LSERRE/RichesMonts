...
<?php get_header(); ?>
			...
			<div id="content" class="montagneBackground">
				<div class="container_12">
					...
					<?php global $post; $terms = get_the_terms( $post->ID , 'recettes_cat' );
					if ( $terms != null ){ ?>
					...
					<section class="the_post_detail">
						<?php if ( have_posts() ) : the_post(); ?>
							...
							<div class="description">
								...
								<?php if(get_field('preparation') || get_field('cuisson') || get_field('nombre_de_convives')){ ?>
									<h3><?php if(get_field('preparation')){ ?> Préparation: <?php the_field('preparation'); ?>min - <?php } if(get_field('preparation')){ ?> Cuisson: <?php the_field('cuisson'); ?>min - <?php } if(get_field('nombre_de_convives')){ ?> Nombre de convives: <?php the_field('nombre_de_convives'); ?> <?php } ?> </h3>
								<?php } ?>
								<?php if(get_field('liste_de_course')){ ?>
									<h4>Ma liste de course:</h4>
									<?php the_field('liste_de_course'); ?>
								<?php } ?>
								<?php if(get_field('produits_conseilles')): ?>
									<div>
										<h4>RichesMonts vous conseille :</h4>
										<?php foreach( get_field('produits_conseilles') as $item ) { ?>
